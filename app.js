...
bot.dialog('ensureProfile', [
    ...
    (session, results) => {
        if (session.message.text.toLowerCase() == `yes`) {
            session.dialogData.profile.confirmation = true;
            builder.Prompts.text(session, `Excellent! Great to meet you.`);
            session.endDialog();
        } else {
            builder.Prompts.text(session, `Sorry about that, let's try again.`);
            session.endDialog();
...
bot.dialog('startSale', [
    ...
    (session, results) => {
        ...
        card.title(`We can help you with your ${results.response.entity}!`);
